...
var loadList = function(first) {layui.config({base : "js/"}).use([ 'form', 'layer', 'jquery', 'laypage', 'table' , 'upload'],
					function() {
						...
						table.render({
							elem : '#list',
							url : ctxPath + '/recruitment_delivery/getAllRecruitmentDelivery' // 数据接口
							,
							cols : [ [ // 表头
							{
								type : 'checkbox',
						...
						table.on('tool(listtable)', function(obj) {
							...
							if (obj.event === 'del') {
								...
								layer.open({
									...
									move : true,
									content : [
											ctxPath + '/recruitment_delivery/showHtmlModify?id='
													+ data.id, 'yes' ],
									end : function() {
									}
								});
						...
						var $ = layui.$, active = {
								...
								index = layer.open({
									...
									type : 2,
									area : [ '100%', '100%' ],
									content : ctxPath + "/recruitment_delivery/showHtmlAdd"
								});
							},
							reload : function() {
								var that = this;
									...
									layer
											.confirm(
													...
													function(index) {
														$.getJSON(
																		ctxPath
																				+ "/recruitment_delivery/deleteSelectRecruitmentDelivery",
																		idDatas,
																		function(
																				jsondata) {
																			if (jsondata.code == '200') {
